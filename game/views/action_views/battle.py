...
class BattleActionView(GameActionView):
    ...
    def _generate_choose_hit_step(self, player_faction, battle, name, endpoint, prompt):
        ...
        buildings = Building.objects.filter(
            building_slot__clearing=battle.clearing, player=player
        )
        ...
        for token in tokens:
            ...
            if token_type_str not in added_piece_names:
                ...
                options.append(
                    {
                        "value": token_type_str,
                        "label": token_type_str,
                        "info": f"Remove a {token_type_str} token to satisfy a hit.",
                    }
                )
        ...
        for building in buildings:
            ...
            if building_type_str not in added_piece_names:
                ...
                options.append(
                    {
                        "value": building_type_str,
                        "label": building_type_str,
                        "info": f"Remove a {building_type_str} building to satisfy a hit.",
                    }
                )
            ...
            payload_details=[
                {
                    "type": "piece",
                    "name": "piece",
                    "clearing_number": battle.clearing.clearing_number,
                }
            ],
            options=options,
        )
    ...
    def post_ambush_check_defender(self, request, game_id: int):
        ...
        battle = Battle.objects.get(event=event)
        try:
            atomic_game_action(defender_ambush_choice, undoable=False)(
                game, battle, card
            )
    ...
    def post_ambush_check_attacker(self, request, game_id: int):
        ...
        battle = Battle.objects.get(event=event)
        try:
            atomic_game_action(attacker_ambush_choice, undoable=False)(
                game, battle, card
            )
        except ValueError as e:
            raise ValidationError({"detail": str(e)})
    ...
    def _process_hit_choice(self, request, game_id, faction_val, hit_func):
        ...
        if not piece_to_remove:
            raise ValidationError(
                f"Piece of type {piece_type_str} not found in battle clearing"
            )
    ...
    def post_choose_ambush_hit_attacker(self, request, game_id: int):
        ...
        battle = Battle.objects.get(event=event)
        return self._process_hit_choice(
            request, game_id, battle.attacker, attacker_choose_ambush_hit
        )
    ...
    def post_choose_hit_defender(self, request, game_id: int):
        ...
        battle = Battle.objects.get(event=event)
        return self._process_hit_choice(
            request, game_id, battle.defender, defender_chooses_hit
        )
    ...
    def post_choose_hit_attacker(self, request, game_id: int):
        ...
        battle = Battle.objects.get(event=event)
        return self._process_hit_choice(
            request, game_id, battle.attacker, attacker_chooses_hit
        )
    ...
    def validate_player(self, request, game_id: int, route: str, *args, **kwargs):
        ...
        if route in ["ambush-check-defender", "choose-hit-defender"]:
            ...
            "ambush-check-attacker",
            "choose-ambush-hit-attacker",
            "choose-hit-attacker",
        ]:
    ...
    def _get_piece_name(self, piece) -> str:
        ...
        if isinstance(piece, Token):
            if hasattr(piece, "keep"):
                return "Keep"
            if hasattr(piece, "wood"):
                return "Wood"
            if hasattr(piece, "sympathy"):
                return "Sympathy"
            return "Token"
            ...
            if hasattr(piece, "birdroost"):
                return "BirdRoost"
            if hasattr(piece, "wabasefox"):
                return "WaBaseFox"
            if hasattr(piece, "wabaserabbit"):
                return "WaBaseRabbit"
            if hasattr(piece, "wabasemouse"):
                return "WaBaseMouse"
            return "Building"
        return "Unknown"
