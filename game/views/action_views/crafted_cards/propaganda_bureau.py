...
class PropagandaBureauView(GameActionView):
            ...
            options.append(
                {
                    ...
                    "info": f"Discard this card to remove an enemy warrior in a {card_ep.value.suit.name} clearing and place one of your warriors there.",
                }
            )
    ...
    def post_pick_card(self, request, game_id):
        ...
        if not valid_clearings:
            raise ValidationError(
                {"detail": "No valid clearings with enemies match that suit"}
            )
    ...
    def post_pick_opponent(self, request, game_id):
        ...
        try:
            atomic_game_action(use_propaganda_bureau)(
                player, card_ep, clearing, target_faction
            )
        except ValueError as e:
            raise ValidationError({"detail": str(e)})
