...
class CrowsGamestateTests(APITestCase):
    def setUp(self):
        ...
        self.cats_client = RootGameClient(
            user=self.cats_user, password="p", game_id=self.game.id
        )
        ...
        self.crows_client = RootGameClient(
            user=self.crows_user, password="p", game_id=self.game.id
        )
        ...
        self.facedown_token = PlotToken.objects.filter(
            player=self.crows_player, plot_type=PlotToken.PlotType.EXTORTION
        ).first()
    ...
    def test_public_gamestate_endpoint(self):
        ...
        self.assertEqual(
            data["reserve_plots_count"], 6
        )  # Starts with 8, 2 moved to board
        ...
        crows_payload = next(
            p for p in data["players"] if p["id"] == self.crows_player.id
        )
    ...
    def test_private_gamestate_endpoint(self):
        ...
        response = self.cats_client.get(
            f"/api/crows/player-private-info/{self.game.id}/"
        )
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

        try:
            response = self.crows_client.get(
                f"/api/crows/player-private-info/{self.game.id}/"
            )
        except Exception as e:
            import traceback
