...
class WABirdsongBaseTestCase(TestCase, LoggingTestMixin):
    def setUp(self):
        ...
        self.game = GameSetupWithFactionsFactory(
            factions=[Faction.CATS, Faction.WOODLAND_ALLIANCE]
        )
        ...
        self.fox_clearing = Clearing.objects.filter(
            game=self.game, suit=Suit.RED
        ).first()
        self.mouse_clearing = Clearing.objects.filter(
            game=self.game, suit=Suit.ORANGE
        ).first()
        self.rabbit_clearing = Clearing.objects.filter(
            game=self.game, suit=Suit.YELLOW
        ).first()
    ...
    def add_supporters(self, suit, count):
        ...
        for _ in range(count):
            ...
            deck_entry = DeckEntry.objects.filter(
                game=self.game, card__suit=suit
            ).first()
            if deck_entry:
...
class WARevoltTests(WABirdsongBaseTestCase):
    def test_revolt_success(self):
        ...
        # Base placed
        self.assertTrue(
            WABase.objects.filter(
                player=self.player,
                suit=Suit.ORANGE,
                building_slot__clearing=self.mouse_clearing,
            ).exists()
        )
        # Enemy pieces removed from board (not necessarily deleted if they return to supply)
        self.assertFalse(
            Warrior.objects.filter(
                player=self.cats_player, clearing=self.mouse_clearing
            ).exists()
        )
        ...
        self.assertEqual(
            Warrior.objects.filter(
                player=self.player, clearing=self.mouse_clearing
            ).count(),
            1,
        )
        # Officer gained
        ...
        self.assertEqual(
            SupporterStackEntry.objects.filter(player=self.player).count(), 0
        )
        ...
        self.assertLogExists(
            LogType.WA_REVOLT,
            player=self.player,
            clearing_number=self.mouse_clearing.clearing_number,
            points_scored=1,
        )
        # Check pieces destroyed in log
        revolt_log = GameLog.objects.filter(
            log_type=LogType.WA_REVOLT, player=self.player
        ).last()
    ...
    def test_revolt_with_bird_supporters(self):
        ...
        self.assertTrue(
            WABase.objects.filter(player=self.player, suit=Suit.ORANGE).exists()
        )
        self.assertEqual(
            SupporterStackEntry.objects.filter(player=self.player).count(), 0
        )
    ...
    def test_revolt_fails_base_already_on_board(self):
        ...
        other_mouse = (
            Clearing.objects.filter(game=self.game, suit=Suit.ORANGE)
            .exclude(id=self.mouse_clearing.id)
            .first()
        )
...
class WASpreadSympathyTests(WABirdsongBaseTestCase):
    ...
    def test_sympathy_cost_scaling(self):
        ...
        for c in clearings:
            ...
            for adj in adjs:
                if not WASympathy.objects.filter(
                    player=self.player, clearing=adj
                ).exists():
                    target = adj
                    break
            if target:
        ...
        spread_sympathy(self.player, target)
        self.assertTrue(
            WASympathy.objects.filter(player=self.player, clearing=target).exists()
        )
    ...
    def test_martial_law(self):
        ...
        WarriorFactory.create_batch(
            3, player=self.cats_player, clearing=self.mouse_clearing
        )
