...
class BattleTransactionTests(TestCase):
    def setUp(self):
        ...
        self.c1 = Clearing.objects.get(
            game=self.game, clearing_number=1
        )  # Fox (Cat Keep)
        ...
        Recruiter.objects.filter(
            building_slot__clearing__in=[self.c1, self.c2]
        ).delete()
    ...
    def test_battle_start_fails_same_faction(self):
        ...
        with self.assertRaises(
            (ValueError, UnavailableActionError, IllegalActionError, InternalGameError)
        ):
    ...
    def test_roll_dice_standard_full_hits(self, mock_randint):
        ...
        self.assertEqual(
            Warrior.objects.filter(player=self.cats, clearing=self.c1).count(), 1
        )
        self.assertEqual(
            Warrior.objects.filter(player=self.birds, clearing=self.c1).count(), 0
        )
    ...
    def test_roll_dice_standard_attacker_capped(self, mock_randint):
        ...
        self.assertEqual(
            Warrior.objects.filter(player=self.birds, clearing=self.c1).count(), 1
        )
    ...
    def test_roll_dice_standard_defender_capped(self, mock_randint):
        ...
        self.assertEqual(battle.attacker_hits_taken, 1)
        self.assertEqual(
            Warrior.objects.filter(player=self.cats, clearing=self.c1).count(), 2
        )
        self.assertEqual(
            Warrior.objects.filter(player=self.birds, clearing=self.c1).count(), 0
        )
    ...
    def test_roll_dice_wa_defender(self, mock_randint):
        ...
        self.assertEqual(
            Warrior.objects.filter(player=self.cats, clearing=self.c1).count(), 0
        )  # 3 - 3 = 0
        self.assertEqual(
            Warrior.objects.filter(player=self.wa, clearing=self.c1).count(), 2
        )  # 3 - 1 = 2
    ...
    def test_roll_dice_eyrie_commander(self, mock_randint):
        ...
        BirdLeader.objects.create(
            player=self.birds, leader=BirdLeader.BirdLeaders.COMMANDER, active=True
        )
    ...
    @patch("game.transactions.battle.randint")
    def test_attacker_ambush_no_counter_losses_1_warrior_and_chooses(
        self, mock_randint
    ):
        mock_randint.side_effect = [0, 0]
        WarriorFactory.create_batch(1, player=self.cats, clearing=self.c1)
    ...
    def test_attacker_choose_ambush_hit_fails_wrong_piece(self, mock_randint):
        ...
        with self.assertRaises(
            (InternalGameError, IllegalActionError, UnavailableActionError)
        ):
    ...
    def test_battle_ambush(self, mock_randint):
        ...
        self.assertEqual(
            battle.step, Battle.BattleSteps.COMPLETED
        )  # Wait, roll_dice might have finished it if results were low.
    ...
    def test_battle_wa_guerilla(self, mock_randint):
        ...
        self.assertEqual(
            Warrior.objects.filter(player=self.cats, clearing=self.c2).count(), 1
        )
        self.assertEqual(
            Warrior.objects.filter(player=self.wa, clearing=self.c2).count(), 1
        )
    ...
    def test_battle_ambush_cancel(self, mock_randint):
        ...
        self.assertEqual(
            Warrior.objects.filter(player=self.cats, clearing=self.c1).count(), 3
        )
        self.assertEqual(battle.step, Battle.BattleSteps.COMPLETED)
    ...
    def test_battle_extra_hits(self, mock_randint):
        ...
        leader = BirdLeader.objects.get(
            player=self.birds, leader=BirdLeader.BirdLeaders.COMMANDER
        )
        leader.active = True
        leader.save()
        ...
        self.assertFalse(
            Recruiter.objects.filter(
                player=self.cats, building_slot__clearing=self.c1
            ).exists()
        )
    ...
    def test_battle_partisans(self):
        ...
        card1 = CardFactory(
            game=self.game, card_type=CardsEP.MOUSE_PARTISANS.name
        )  # Mouse (suit o)
        ...
        self.assertFalse(
            HandEntry.objects.filter(player=self.cats, card=card1).exists()
        )
        # Battle completed
        self.assertEqual(battle.step, Battle.BattleSteps.COMPLETED)
