...
class CatBaseTestCase(TestCase, LoggingTestMixin):
    def setUp(self):
        ...
        )  # Fox (Cat Keep / Sawmill)
        self.c5 = Clearing.objects.get(
            game=self.game, clearing_number=5
        )  # Rabbit (Workshop)
        self.c9 = Clearing.objects.get(
            game=self.game, clearing_number=9
        )  # Mouse (Recruiter)
...
class CatRecruitTests(CatBaseTestCase):
    def test_recruit_all(self):
        ...
        initial_w9 = Warrior.objects.filter(
            player=self.player, clearing=self.c9
        ).count()
        initial_w5 = Warrior.objects.filter(
            player=self.player, clearing=self.c5
        ).count()

        cat_recruit_all(self.player)

        self.assertEqual(
            Warrior.objects.filter(player=self.player, clearing=self.c9).count(),
            initial_w9 + 1,
        )
        self.assertEqual(
            Warrior.objects.filter(player=self.player, clearing=self.c5).count(),
            initial_w5 + 1,
        )
    ...
    def test_recruit_no_actions_fails(self):
        ...
        self.daylight.save()
        all_on_board = Recruiter.objects.filter(
    ...
    def test_recruit_success_updates_state(self):
        target_recruiter = Recruiter.objects.filter(
            player=self.player, building_slot__isnull=False
        )
        cat_recruit(self.player, target_recruiter)

        self.assertLogExists(
            LogType.CATS_RECRUIT,
            player=self.player,
            total_warriors=target_recruiter.count(),
        )
...
class CatBuildTests(CatBaseTestCase):
    def test_build_workshop_success(self):
        ...
        self.assertLogExists(
            ...
            clearing_number=self.c9.clearing_number,
        )
...
class CatOverworkTests(CatBaseTestCase):
    def test_overwork_success(self):
        ...
        initial_wood = CatWood.objects.filter(
            player=self.player, clearing=self.c1
        ).count()
        overwork(self.player, self.c1, card_enum)

        self.assertEqual(
            CatWood.objects.filter(player=self.player, clearing=self.c1).count(),
            initial_wood + 1,
        )
        self.assertFalse(
            HandEntry.objects.filter(player=self.player, card=card_obj).exists()
        )

        self.assertLogExists(
            LogType.CATS_OVERWORK,
            player=self.player,
...
class CatMovementTests(CatBaseTestCase):
    def test_march_uses_one_action_for_two_moves(self):
        ...
        self.assertLogExists(
            LogType.MOVE,
            player=self.player,
            origin_clearing_number=self.c1.clearing_number,
            dest_clearing_number=self.c5.clearing_number,
        )
    ...
    def test_crafting_rabbit_partisans_success(self):
        ...
        self.assertTrue(
            CraftedCardEntry.objects.filter(
                player=self.player, card__card_type=card_enum.name
            ).exists()
        )
