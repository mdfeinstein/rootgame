...
class CrowBirdsongTestCase(LoggingTestMixin, TestCase):
    ...
    def test_flip_plot_bomb_resolves(self):
        ...
        flip_plot(self.player, p1)
        self.assertEqual(
            Warrior.objects.filter(player=self.cats_player, clearing=self.c2).count(), 0
        )

        # Verify Logs
        self.assertLogExists(LogType.CROWS_FLIP, player=self.player, plot_type="bomb")
        self.assertLogExists(
            LogType.PIECE_REMOVAL, player=self.player, count=2, piece_type="Warrior"
        )

    def test_flip_plot_extortion_resolves(self):
        p1 = PlotToken.objects.filter(
            player=self.player, plot_type=PlotToken.PlotType.EXTORTION
        ).first()
        p1.clearing = self.c2
        p1.is_facedown = True
        p1.save()
        ...
        # Check card stolen
        self.assertTrue(
            HandEntry.objects.filter(player=self.player, card=cat_card).exists()
        )
        # Verify Logs
        self.assertLogExists(
            LogType.CROWS_EXTORTION_STOLE_CARD,
            player=self.player,
            victim_faction=Faction.CATS.value,
        )
    ...
    def test_recruit_success(self):
        ...
        # Verify logs
        self.assertLogExists(
            LogType.CROWS_RECRUIT, player=self.player, clearing_count=len(fox_clearings)
        )
