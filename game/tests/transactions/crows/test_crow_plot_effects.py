...
class CrowPlotTokenEffectsTests(APITestCase, LoggingTestMixin):
    ...
    def test_bomb_self_removal(self):
        ...
        self.assertEqual(
            Warrior.objects.filter(clearing=self.c1, player=self.cats_player).count(), 0
        )
    ...
    def test_snare_placement_block(self):
        ...
        snare = PlotToken.objects.filter(
            player=self.crows_player, plot_type=PlotToken.PlotType.SNARE
        ).first()
    ...
    def test_raid_auto_placement(self):
        ...
        raid = PlotToken.objects.filter(
            player=self.crows_player, plot_type=PlotToken.PlotType.RAID
        ).first()
        ...
        initial_warriors_in_c5 = Warrior.objects.filter(
            clearing=self.c5, player=self.crows_player
        ).count()

        player_removes_token(self.game, raid, self.cats_player, is_exposure=False)

        self.assertEqual(
            Warrior.objects.filter(clearing=self.c5, player=self.crows_player).count(),
            initial_warriors_in_c5 + 1,
        )
        # VERIFY LOG
    ...
    def test_raid_manual_placement_event(self):
        ...
        event = Event.objects.filter(
            game=self.game, type=EventType.PLACE_RAID_WARRIORS, is_resolved=False
        ).first()
