...
class CrowActionsTestCase(LoggingTestMixin, TestCase):
    def setUp(self):
        ...
        c1_ids = list(
            Warrior.objects.filter(player=self.player).values_list("id", flat=True)[:5]
        )
    ...
    def test_crows_plot_success_and_cost_scaling(self):
        ...
        self.assertEqual(
            Warrior.objects.filter(clearing=self.c2, player=self.player).count(), 1
        )
        ...
        self.assertLogExists(
            LogType.CROWS_PLOT,
            player=self.player,
            clearing_number=self.c2.clearing_number,
            plot_type=plot_type,
        )
        self.assertLogExists(
            LogType.PIECE_REMOVAL, player=self.player, count=1, piece_type="Warrior"
        )

        # make sure only one warrior in c1, and no catkeep
        CatKeep.objects.filter(clearing=self.c1).delete()
        Warrior.objects.filter(clearing=self.c1, player=self.player).update(
            clearing=None
        )
        place_warriors_into_clearing(self.player, self.c1, 1)
        self.assertEqual(
            Warrior.objects.filter(clearing=self.c1, player=self.player).count(), 1
        )
        # Another plot should cost 2, so this should fail
        with self.assertRaises(IllegalActionError):
            crows_plot(
                self.player, self.c1, PlotToken.PlotType.SNARE, daylight=self.daylight
            )
        ...
        self.assertEqual(
            Warrior.objects.filter(clearing=self.c1, player=self.player).count(), 0
        )

    def test_crows_plot_failures(self):
        # 1. token not in supply
        PlotToken.objects.filter(
            player=self.player, plot_type=PlotToken.PlotType.BOMB
        ).update(clearing=self.c1)
        with self.assertRaises(IllegalActionError):
        ...
        PlotToken.objects.filter(
            player=self.player, plot_type=PlotToken.PlotType.SNARE
        ).update(clearing=self.c2)
        with self.assertRaises(IllegalActionError):
            crows_plot(self.player, self.c2, PlotToken.PlotType.BOMB)
    ...
    def test_crows_move_ignores_rule(self):
        ...
        self.assertEqual(
            Warrior.objects.filter(clearing=self.c2, player=self.player).count(), 3
        )
        ...
        self.assertLogExists(
            ...
            dest_clearing_number=self.c2.clearing_number,
            warriors_moved=1,
        )

    def test_crows_trick_both_facedown_success(self):
        p1 = PlotToken.objects.filter(
            player=self.player, plot_type=PlotToken.PlotType.BOMB
        ).first()
        p2 = PlotToken.objects.filter(
            player=self.player, plot_type=PlotToken.PlotType.SNARE
        ).first()
        ...
        self.assertLogExists(
            LogType.CROWS_TRICK,
            player=self.player,
            clearing1=self.c1.clearing_number,
            clearing2=self.c2.clearing_number,
        )
