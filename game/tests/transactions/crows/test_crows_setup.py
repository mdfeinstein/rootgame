...
class CrowPlaceWarriorTests(CrowSetupBaseTestCase):
    def test_place_warrior_success(self):
        # Find clearings
        fox_c = (
            Clearing.objects.filter(game=self.game, suit=Suit.RED.value)
            .exclude(clearing_number=1)
            .first()
        )
        rabbit_c = Clearing.objects.filter(
            game=self.game, suit=Suit.YELLOW.value
        ).first()
        mouse_c = Clearing.objects.filter(
            game=self.game, suit=Suit.ORANGE.value
        ).first()

        # Place in Fox
        place_initial_warrior(self.player, fox_c)
        self.assertEqual(
            Warrior.objects.filter(clearing=fox_c, player=self.player).count(), 1
        )
        ...
        self.assertEqual(
            self.crow_setup.step, CrowsSimpleSetup.Steps.PENDING_CONFIRMATION
        )

        # Verify logs
        self.assertLogExists(
            LogType.CROWS_SETUP_PLACE_WARRIOR,
            player=self.player,
            clearing_number=fox_c.clearing_number,
            suit=Suit.RED.value,
        )
        self.assertLogExists(
            LogType.CROWS_SETUP_PLACE_WARRIOR,
            player=self.player,
            clearing_number=rabbit_c.clearing_number,
            suit=Suit.YELLOW.value,
        )
        self.assertLogExists(
            LogType.CROWS_SETUP_PLACE_WARRIOR,
            player=self.player,
            clearing_number=mouse_c.clearing_number,
            suit=Suit.ORANGE.value,
        )
