...
class CrowsExposureAndAgentsTestCase(LoggingTestMixin, TestCase):
    def setUp(self):
        ...
        self.token = PlotToken.objects.filter(
            player=self.crows_player, plot_type=PlotToken.PlotType.BOMB
        ).first()
    ...
    def test_can_attempt_exposure_conditions(self):
        ...
        c_warrior = Warrior.objects.filter(
            player=self.cats_player, clearing=self.c1
        ).first()
    ...
    def test_exposure_incorrect_guess(self):
        hand_entry = HandEntry.objects.create(
            player=self.cats_player, card=self.fox_card
        )
        ...
        self.assertLogExists(
            LogType.CROWS_EXPOSURE,
            player=self.cats_player,
            clearing_number=self.c1.clearing_number,
            guessed_plot_type="snare",
            success=False,
        )
    ...
    def test_exposure_correct_guess(self):
        ...
        self.assertLogExists(
            ...
            player=self.cats_player,
            clearing_number=self.c1.clearing_number,
            guessed_plot_type="bomb",
            success=True,
        )

    def test_exposure_failures(self):
        hand_entry_fox = HandEntry.objects.create(
            player=self.cats_player, card=self.fox_card
        )
        hand_entry_mouse = HandEntry.objects.create(
            player=self.cats_player, card=self.mouse_card
        )

        # Wrong suit
        with self.assertRaises(IllegalActionError):
            guess_exposure(
                self.cats_player, self.c1, hand_entry_mouse, PlotToken.PlotType.BOMB
            )
        ...
        with self.assertRaises(IllegalActionError):
            guess_exposure(
                self.cats_player, self.c1, hand_entry_fox, PlotToken.PlotType.BOMB
            )
