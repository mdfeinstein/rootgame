...
class CrowEveningTestCase(LoggingTestMixin, TestCase):
    ...
    def test_do_exert_action_marks_used(self):
        ...
        do_exert_action(
            self.player, "move", origin=self.c1, destination=self.c2, count=1
        )
    ...
    def test_plot_in_evening_uses_daylight_cost(self):
        ...
        self.assertEqual(
            Warrior.objects.filter(clearing=self.c1, player=self.player).count(), 2
        )  # Started with 5
