...
class AdjacencyPassivesTestCase(TestCase):
    ...
    def test_boat_builders_adjacency(self):
        ...
        card = Card.objects.filter(
            game=self.game, card_type=CardsEP.BOAT_BUILDERS.name
        ).first()
    ...
    def test_tunnels_adjacency(self):
        ...
        # Give Cats Tunnels
        card = Card.objects.filter(
            game=self.game, card_type=CardsEP.TUNNELS.name
        ).first()
        CraftedCardEntry.objects.create(player=self.cats_player, card=card)
    ...
    def test_sympathy_spread_tunnels(self):
        ...
        for _ in range(5):
            SupporterStackEntry.objects.create(
                player=self.wa_player, card=Card.objects.filter(game=self.game).first()
            )
    ...
    def test_corvid_planners_movement(self):
        ...
        card = Card.objects.filter(
            game=self.game, card_type=CardsEP.CORVID_PLANNERS.name
        ).first()
        CraftedCardEntry.objects.create(player=self.cats_player, card=card)
    ...
    def test_soup_kitchens_rule(self):
        ...
        card = Card.objects.filter(
            game=self.game, card_type=CardsEP.SOUP_KITCHENS.name
        ).first()
        CraftedCardEntry.objects.create(player=self.cats_player, card=card)
