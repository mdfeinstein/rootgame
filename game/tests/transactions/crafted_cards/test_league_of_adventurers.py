...
class TestLeagueOfAdventurersTransaction(TestCase):
    def setUp(self):
        ...
        self.league_card = CardFactory(
            game=self.game, card_type=CardsEP.LEAGUE_OF_ADVENTURERS.name
        )
    ...
    def test_use_league_of_adventurers_move_success(self):
        ...
        use_league_of_adventurers(
            self.crafted_league, self.crafted_item, move_data=move_data
        )
        ...
        self.assertEqual(
            Warrior.objects.filter(player=self.player, clearing=clearing1).count(), 1
        )
        self.assertEqual(
            Warrior.objects.filter(player=self.player, clearing=clearing2).count(), 2
        )
    ...
    def test_use_league_of_adventurers_battle_success(self):
        ...
        battle_data = {"clearing": clearing1, "opponent_faction": self.opponent.faction}

        use_league_of_adventurers(
            self.crafted_league, self.crafted_item, battle_data=battle_data
        )
        ...
        self.assertTrue(
            Battle.objects.filter(
                clearing=clearing1,
                attacker=self.player.faction,
                defender=self.opponent.faction,
            ).exists()
        )
    ...
    def test_fails_if_wrong_phase(self):
        ...
        BirdDaylight.objects.filter(turn=self.turn).update(
            step=BirdDaylight.BirdDaylightSteps.COMPLETED
        )
        BirdEvening.objects.filter(turn=self.turn).update(
            step=BirdEvening.BirdEveningSteps.SCORING
        )
