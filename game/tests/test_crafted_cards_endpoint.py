...
from game.models.game_models import Game, Player, Faction, Card, CraftedCardEntry, DeckEntry
...
class TestCraftedCardsEndpoint(TestCase):
    def setUp(self):
        ...
        self.bird_turn = BirdTurn.objects.create(
            player=self.player,
            turn_number=1
        )
        self.birdson = BirdBirdsong.objects.create(
            turn=self.bird_turn,
            step="1" # Start of birdsong for Saboteurs test
        )
        self.daylight = BirdDaylight.objects.create(
            turn=self.bird_turn,
            step="0"
        )
        self.evening = BirdEvening.objects.create(
            turn=self.bird_turn,
            step="0"
        )
        ...
        self.saboteurs_card = Card.objects.create(
            game=self.game,
            suit="Bird",
            card_type=CardsEP.SABOTEURS.name
        )
        self.informants_card = Card.objects.create(
             game=self.game,
             suit="Fox",
             card_type=CardsEP.INFORMANTS.name
        )
    ...
    def test_get_crafted_cards(self):
        response = self.client.get(f"/api/crafted-cards/{self.game.pk}/Birds/")
        ...
        saboteurs = next(item for item in data if item['card']['card_name'] == 'SABOTEURS' and item['used'] is False)
        informants = next(item for item in data if item['card']['card_name'] == 'INFORMANTS')
