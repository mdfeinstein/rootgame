...
def use_saboteurs(player: Player, target_crafted_card: CraftedCardEntry):
    ...
    if not can_use_card(player, saboteurs_entry):
        raise UnavailableActionError(
            "Saboteurs cannot be used right now. It must be at the start of your Birdsong."
        )

    # 3. Validation: Target card must be in the same game
    if player.game != target_crafted_card.player.game:
        raise UnavailableActionError("Target card is not in the same game.")

    # 4. Check if player owns the target card
    if target_crafted_card.player == player:
        raise IllegalActionError(
            "You cannot discard your own crafted card with Saboteurs."
        )
...
def saboteurs_check(player: Player) -> bool:
    ...
    saboteurs_entry = CraftedCardEntry.objects.filter(
        player=player,
        card__card_type=CardsEP.SABOTEURS.name,
...
def saboteurs_skip(player: Player):
    ...
    """
    saboteurs_entry = CraftedCardEntry.objects.filter(
        player=player, card__card_type=CardsEP.SABOTEURS.name
    ).first()
    if saboteurs_entry is None:
        raise UnavailableActionError("Player does not have Saboteurs")

    # mark as used
    saboteurs_entry.used = CraftedCardEntry.UsedChoice.USED
    saboteurs_entry.save()

    event = SaboteursEvent.objects.filter(
        crafted_card_entry=saboteurs_entry, event__is_resolved=False
    ).first()
    if event:
        event.event.is_resolved = True
        event.event.save()
