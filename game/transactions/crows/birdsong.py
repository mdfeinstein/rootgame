...
def crows_recruit(player: Player, card: CardsEP, suit: Suit | None = None):
    ...
    valid_clearings = list(
        Clearing.objects.filter(game=player.game, suit=target_suit.value)
    )
    ...
    if warriors_in_supply < len(valid_clearings):
        ...
        event = Event.objects.create(game=player.game, type=EventType.CROW_RECRUIT)
...
def manual_recruit(player: Player, clearing: Clearing, event: Event):
    ...
    valid_clearings_left = Clearing.objects.filter(
        game=player.game, suit=recruit_event.suit
    ).exclude(
        id__in=recruit_event.recruited_clearings.all().values_list("id", flat=True)
    )
